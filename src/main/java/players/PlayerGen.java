...
public class PlayerGen {
  ...
  public ArrayList<Player> getPlayersFromFile() {
    final ArrayList<Player> PlayerList = new ArrayList<Player>();
    try {
      final List<String> playerLine = Files.readAllLines(Paths
          .get("player_stats.txt"));
      for (final String line : playerLine) {
        ...
        ratings[2] = Math.round(50 + 65 * Float.parseFloat(stats[8]) + 3
            * Float.parseFloat(stats[6]));
        if (Float.parseFloat(stats[9]) > 0.3) {
          ratings[4] = Math.round(50 + 70 * Float.parseFloat(stats[11]) + 6
              * Float.parseFloat(stats[9]));
        } else {
          ratings[4] = 30;
        }

        ratings[3] = (int) Math.round((ratings[2] + ratings[4]) / 2.7
            * (0.45 + Float.parseFloat(stats[17])));
        ratings[5] = 60 + Math.round(10 * Float.parseFloat(stats[21])
            / Float.parseFloat(stats[24]) + Float.parseFloat(stats[21]));
        ratings[6] = 75;
        ...
        ratings[9] = (int) Math.round((ratings[8] + ratings[11]) / 2.1);
        ratings[10] = Math
            .round((float) (ratings[7] + ratings[4] + ratings[5]) / 3);
        ratings[13] = Math.round((Float.parseFloat(stats[28]) + Float
            .parseFloat(stats[29])) * 1000);
        ratings[14] = Math.round((Float.parseFloat(stats[30]) + Float
            .parseFloat(stats[31])) * 1000);
        ...
        ratings[1] = (int) Math.round(Math.pow(ratings[2], 1.3)
            + Math.pow(ratings[3], 1.3) + Math.pow(ratings[4], 1.3)
            + Math.pow(ratings[5], 1.1) + ratings[6]
            + Math.pow(ratings[7], 1.1) + Math.pow(ratings[8], 1.1)
            + Math.pow(ratings[9], 1.2) + Math.pow(ratings[10], 1.2)
            + Math.pow(ratings[11], 1.2));
        ratings[1] = 100 * ratings[1] / 2500;

        final String name = stats[0] + " " + stats[1] + " " + stats[2];
        final Player draftPlayer = new Player(name, ratings);
